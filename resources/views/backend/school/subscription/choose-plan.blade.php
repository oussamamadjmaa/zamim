...
@section('content')
    <div class="container">
        <h1 class="text-center py-5">@lang('Choose your subscription plan')</h1>
                    ...
                    <div class="subscription-plan">
                        <div class="subscription-plan__heading">
                            <div class="d-flex">
                                ...
                                <h2 class="plan-price">{{ number_format($plan['price'], 0) }}<span class="currency">
                                        @lang($plan['currency'])</span></h2>
                            </div>
                                        ...
                                        @if (is_array($feature))
                                            <ul class="list-unstyled mb-3 ps-3">
                                                @foreach ($feature as $featureSecTitle)
                                                    <li> <ion-icon name="checkmark-outline" class="me-2"></ion-icon>
                                                        {{ $featureSecTitle }}</li>
                                                @endforeach
                                            </ul>
                                        @endif
                        ...
                        <a href="{{ route('school.subscription.checkout', ['plan_id' => $plan->id]) }}"
                            class="primary-button d-block py-3 fs-5 w-100">@lang('Subscribe now')</a>
                    </div>
